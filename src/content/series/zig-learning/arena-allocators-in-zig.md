...
<figure class="plate-scroll">
  <img class="plate-light" src="/images/zig/arena-vs-manual.svg" alt="Two panels. Left: the testing allocator hands out five separate objects from gpa, with a three-step teardown that must free the borrower before the owners. Right: the arena takes big blocks from gpa and packs the same five objects inside, freed all at once by a single deinit.">
  <img class="plate-dark" src="/images/zig/arena-vs-manual-dark.svg" alt="Two panels. Left: the testing allocator hands out five separate objects from gpa, with a three-step teardown that must free the borrower before the owners. Right: the arena takes big blocks from gpa and packs the same five objects inside, freed all at once by a single deinit.">
  <figcaption><strong>The same allocations, two lifetimes.</strong> (a) Under the tracking allocator every object is reclaimed individually, so teardown has an <em>order</em>: <code>topK</code>'s results borrow the map's keys, so the borrower has to go first. That ordering is a real invariant you can get wrong. (b) The arena takes big blocks and sub-allocates inside them; individual <code>free</code> is a no-op, and one <code>deinit</code> drops everything. Neither <code>countWords</code> nor <code>topK</code> changes between the two — <em>the strategy is chosen entirely by what the caller passes in.</em></figcaption>
</figure>
